...
// fold walks the buffer with unsafe.Add rather than re-slicing it at every row, and stops maxRow short of the end so every load in the loop is unconditional; foldTail closes the rest without ever over-reading.
//
// What keeps the names handed to update pointing at live memory is the CALLER: foldRange owns buf and it outlives this call.
func (t *table) fold(data []byte, base int64) error {
	pos, n := 0, len(data)
	p := unsafe.Pointer(unsafe.SliceData(data))
	for pos+maxRow <= n {
		row := unsafe.Add(p, pos)
		sep, semi := indexDelimAt(row, n-pos)
		if sep < 0 || !semi {
			return rowError(base+int64(pos), data[pos:])
		}
		// This guard is the ONLY thing bounding the load below, which reads past the buffer in silence rather than panicking.
		if pos+sep+9 > n {
			break
		}
		v, next, ok := parseTemp(*(*uint64)(unsafe.Add(row, sep+1)))
		if !ok || uint32(v+999) > 1998 {
			return rowError(base+int64(pos), data[pos:])
		}
		kw := maskWord(*(*uint64)(row), sep)
		if !t.update(mixWord(kw), unsafe.Slice((*byte)(row), sep), v) {
...
// indexDelimAt finds the first ';' or '\n' eight bytes at a time over a raw pointer.
//
...
func indexDelimAt(p unsafe.Pointer, n int) (idx int, semi bool) {
	i := 0
	for ; i+8 <= n; i += 8 {
